...
%% INPUT PARAMETERS
 ...
 % Fixed technical parameters  
    ...
    HHV         = 39.4 * 3.6 * 10^3;           % (141.9 MJ/kg) Hydrogen higher heating value [kJ/kg]  
    LHV         = 33.3 * 3.6 * 10^3;           % (120.1 MJ/kg) Hydrogen lower heating value [kJ/kg]
...
% OPERATIONAL decision variables
   ...
   m_flow_H2 = P_FC./eff_FC./HHV.*3600 + Const1 *w_hours;
%% OBJECTIVE FUNCTION

cost_inst     = (P_PV_peak * UP_PV * ann_PV + S_FC * UP_FC * ann_FC + C_b/3600 * UP_b * ann_b)/1000; % [kEUR/y]
cost_imp      = sum(c_h2 * m_flow_H2)/1000 + sum(P_imp .* c_gridimp) / 1000;
...
cost_inst_2       = evaluate(cost_inst, solution);                                 % kEUR/y
cost_imp_2        = evaluate(cost_imp, solution);                                  % kEUR/y
cost_maint_2      = evaluate(cost_maint, solution);                                % kEUR/y
...
m_flow_H2_opt   = P_FC_opt./eff_FC./HHV.*3600 + Const1.*w_hours_opt;
m_flow_H2_id   = P_FC_opt./eff_FC./HHV.*3600; % [kg/h]
   
etadeg_opt      = P_FC_opt./m_flow_H2_opt./HHV*3600; 

save('scenariocost2.mat', 'cost_inst_2', 'cost_imp_2', 'cost_exp_2', 'cost_maint_2', 'cost_tot_2');
...
%energy indicators
    % design indicators 
    R_FC  = S_FC_opt*eff_FC/mean(P_load); % [-] fuel cell ratio
    % energy indicators
