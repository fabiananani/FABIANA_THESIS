...
figure
h1=area(tt,-P_PV_opt(start:finish), 'FaceColor', cmap(colors(2), :), 'FaceAlpha', 0.3, 'EdgeColor', 'none');
hold on
plot(tt,-P_PV_opt(start:finish),'LineWidth',linew,'Color',cmap(colors(2), :));
hold on
h2=area(tt, -P_FC_opt(start:finish), 'FaceColor', cmap(colors(3), :), 'FaceAlpha', 0.3, 'EdgeColor', 'none');
hold on
plot(tt,-P_FC_opt(start:finish),'LineWidth',linew,'Color',cmap(colors(3), :));
hold on
h3=area(tt, -P_imp_opt(start:finish), 'FaceColor', cmap(colors(4), :), 'FaceAlpha', 0.3, 'EdgeColor', 'none');
hold on
plot(tt,-P_imp_opt(start:finish),'LineWidth',linew,'Color',cmap(colors(4), :));
hold on 
h4=area(tt,P_exp_opt(start:finish), 'FaceColor', cmap(colors(5), :), 'FaceAlpha', 0.3, 'EdgeColor', 'none');
hold on 
plot(tt,P_exp_opt(start:finish),'LineWidth',linew,'Color',cmap(colors(5), :));
hold on
h5=area(tt, P_load(start:finish), 'FaceColor', cmap(colors(6), :), 'FaceAlpha', 0.3, 'EdgeColor', 'none');
hold on
plot(tt,P_load(start:finish),'LineWidth',linew,'Color',cmap(colors(6), :));
hold on
...
% xlabel('Time','fontweight','bold','FontName','Time','Times New Roman');
legend([h1, h2, h3, h4, h5],'P_{PV}','P_{FC}','P_{imp}','P_{exp}','P_{load}','location','eastoutside','FontName','Times New Roman')

set(gcf, 'Units', 'inches');
set(gcf, 'Position', [0, 0, 7, 4.5]); % Width=3.5in, Height=2.5in
set(gca, 'FontSize', font-8,'FontName','Times New Roman'); % Set axis font size and font name
